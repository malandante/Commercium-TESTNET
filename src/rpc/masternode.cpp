...
UniValue masternode(const JSONRPCRequest& request)
{
    ...
    if (strCommand == "current" || strCommand == "winner")
    {
        ...
        obj.push_back(Pair("outpoint",      mnInfo.outpoint.ToStringShort()));
        obj.push_back(Pair("payee",         CCommerciumAddress(mnInfo.pubKeyCollateralAddress.GetID()).ToString()));
    ...
    if (strCommand == "genkey")
    {
        ...
        return CCommerciumSecret(secret).ToString();
    }
    ...
    if (strCommand == "status")
    {
        ...
        if(mnodeman.Get(activeMasternode.outpoint, mn)) {
            mnObj.push_back(Pair("payee", CCommerciumAddress(mn.pubKeyCollateralAddress.GetID()).ToString()));
        }
...
UniValue masternodelist(const JSONRPCRequest& request)
{
                ...
                strMode != "rank" && strMode != "sentinel" && strMode != "status"))
    {
        throw std::runtime_error(
                ...
                "  payee          - Print Commercium address associated with a masternode (can be additionally filtered,\n"
                "                   partial match)\n"
        ...
        for (const auto& mnpair : mapMasternodes) {
            ...
            if (strMode == "activeseconds") {
                ...
                streamFull << std::setw(18) <<
                               mn.GetStatus() << " " <<
                               mn.nProtocolVersion << " " <<
                               CCommerciumAddress(mn.pubKeyCollateralAddress.GetID()).ToString() << " " <<
                ...
                objMN.push_back(Pair("payee", CCommerciumAddress(mn.pubKeyCollateralAddress.GetID()).ToString()));
                ...
                CCommerciumAddress address(mn.pubKeyCollateralAddress.GetID());
    ...
    if (strCommand == "decode")
    {
        ...
        for (const auto& mnb : vecMnb) {
            ...
            if(mnb.CheckSignature(nDos)) {
                ...
                resultObj.push_back(Pair("pubKeyCollateralAddress", CCommerciumAddress(mnb.pubKeyCollateralAddress.GetID()).ToString()));
                resultObj.push_back(Pair("pubKeyMasternode", CCommerciumAddress(mnb.pubKeyMasternode.GetID()).ToString()));
...
{ //  category              name                      actor (function)         okSafe argNames
  //  --------------------- ------------------------  -----------------------  ------ ----------
    ...
    { "commercium",               "masternodelist",         &masternodelist,         true,  {} },
    { "commercium",               "masternodebroadcast",    &masternodebroadcast,    true,  {} },
    { "commercium",               "getpoolinfo",            &getpoolinfo,            true,  {} },
    { "commercium",               "sentinelping",           &sentinelping,           true,  {} },
#ifdef ENABLE_WALLET
    { "commercium",               "privatesend",            &privatesend,            false, {} },
#endif // ENABLE_WALLET
};
