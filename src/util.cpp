...
const char * const BITCOIN_CONF_FILENAME = "commercium.conf";
const char * const BITCOIN_PID_FILENAME = "commerciumd.pid";
    ...
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            if (mapMultiArgs.count("-debug")) {
                ...
                // "commercium" is a composite category enabling all Commercium-related debug output
                if(ptrCategory->count(std::string("commercium"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\CommerciumCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\CommerciumCore
    // Mac: ~/Library/Application Support/CommerciumCore
    // Unix: ~/.commerciumcore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "CommerciumCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/CommerciumCore";
#else
    // Unix
    return pathRet / ".commerciumcore";
#endif
#endif
}
...
void ReadConfigFile(const std::string& confPath)
{
    ...
    if (!streamConfig.good()){
        // Create empty commercium.conf if it does not excist
