...
/** 
 * Fees smaller than this (in commercii) are considered zero fee (for transaction creation)
...
bool CWallet::LoadCScript(const CScript& redeemScript)
{
    ...
    {
        std::string strAddr = CCommerciumAddress(CScriptID(redeemScript)).ToString();
...
bool CWallet::SelectCoinsByDenominations(int nDenom, CAmount nValueMin, CAmount nValueMax, std::vector<CTxDSIn>& vecTxDSInRet, std::vector<COutput>& vCoinsRet, CAmount& nValueRet, int nPrivateSendRoundsMin, int nPrivateSendRoundsMax)
{
    ...
    // ( bit on if present )
    // bit 0 - 100COMMERCIUM+1
    // bit 1 - 10COMMERCIUM+1
    // bit 2 - 1COMMERCIUM+1
    // bit 3 - .1COMMERCIUM+1
...
bool CWallet::SelectCoinsGrouppedByAddresses(std::vector<CompactTallyItem>& vecTallyRet, bool fSkipDenominated, bool fAnonymizable, bool fSkipUnconfirmed) const
{
    ...
    if (LogAcceptCategory("selectcoins")) {
        ...
        for (const auto& item : vecTallyRet)
            strMessage += strprintf("  %s %f\n", CCommerciumAddress(item.txdest).ToString().c_str(), float(item.nAmount)/COIN);
        ...
        LOCK2(cs_main, cs_wallet);
        {
            ...
            while (true)
            {
                ...
                if (fUseInstantSend && nValueIn > sporkManager.GetSporkValue(SPORK_5_INSTANTSEND_MAX_VALUE)*COIN) {
                    strFailReason += " " + strprintf(_("InstantSend doesn't support sending values that high yet. Transactions are currently limited to %1 COMMERCIUM."), sporkManager.GetSporkValue(SPORK_5_INSTANTSEND_MAX_VALUE));
                    return false;
                }
                ...
                if (nChange > 0)
                {
                    ...
                    if (nCoinType == ONLY_DENOMINATED) {
                        ...
                        // change transaction isn't always pay-to-commercium-address
                        CScript scriptChange;
...
bool CWallet::SetAddressBook(const CTxDestination& address, const std::string& strName, const std::string& strPurpose)
{
    ...
    if (!fFileBacked)
        return false;
    if (!strPurpose.empty() && !CWalletDB(strWalletFile).WritePurpose(CCommerciumAddress(address).ToString(), strPurpose))
        return false;
    return CWalletDB(strWalletFile).WriteName(CCommerciumAddress(address).ToString(), strName);
}
...
{
    {
        ...
        if(fFileBacked)
        {
            ...
            std::string strAddress = CCommerciumAddress(address).ToString();
    ...
    if (!fFileBacked)
        return false;
    CWalletDB(strWalletFile).ErasePurpose(CCommerciumAddress(address).ToString());
    return CWalletDB(strWalletFile).EraseName(CCommerciumAddress(address).ToString());
}
...
bool CWallet::AddDestData(const CTxDestination &dest, const std::string &key, const std::string &value)
{
    ...
    if (!fFileBacked)
        return true;
    return CWalletDB(strWalletFile).WriteDestData(CCommerciumAddress(dest).ToString(), key, value);
}
...
bool CWallet::EraseDestData(const CTxDestination &dest, const std::string &key)
{
    ...
    if (!fFileBacked)
        return true;
    return CWalletDB(strWalletFile).EraseDestData(CCommerciumAddress(dest).ToString(), key);
}
