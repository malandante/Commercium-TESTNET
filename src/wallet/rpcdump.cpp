...
void ImportAddress(const CCommerciumAddress& address, const std::string& strLabel);
void ImportScript(const CScript& script, const std::string& strLabel, bool isRedeemScript)
{
    ...
    if (isRedeemScript) {
        ...
        ImportAddress(CCommerciumAddress(CScriptID(script)), strLabel);
    } else {
...
UniValue importpubkey(const JSONRPCRequest& request)
{
    ...
    ImportAddress(CCommerciumAddress(pubKey.GetID()), strLabel);
...
UniValue importwallet(const JSONRPCRequest& request)
{
    ...
    while (file.good()) {
        ...
        if (pwalletMain->HaveKey(keyid)) {
            LogPrintf("Skipping import of %s (key already present)\n", CCommerciumAddress(keyid).ToString());
            continue;
        }
...
UniValue dumpprivkey(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            ...
            "\nArguments:\n"
            "1. \"address\"   (string, required) The commercium address for the private key\n"
            "\nResult:\n"
...
UniValue dumpwallet(const JSONRPCRequest& request)
{
    ...
    if (pwalletMain->GetHDChain(hdChainCurrent))
    {
        ...
        CCommerciumExtPubKey b58extpubkey;
    ...
    for (std::vector<std::pair<int64_t, CKeyID> >::const_iterator it = vKeyBirth.begin(); it != vKeyBirth.end(); it++) {
        ...
        std::string strAddr = CCommerciumAddress(keyid).ToString();
        CKey key;
        if (pwalletMain->GetKey(keyid, key)) {
            file << strprintf("%s %s ", CCommerciumSecret(key).ToString(), strTime);
...
UniValue ProcessImport(const UniValue& data, const int64_t timestamp)
{
    try {
        ...
        if (!isScript) {
            address = CCommerciumAddress(output);
            if (!address.IsValid()) {
        ...
        if (isP2SH) {
            ...
            CCommerciumAddress redeemAddress = CCommerciumAddress(CScriptID(redeemScript));
            ...
            if (pubKeys.size() && keys.size() == 0) {
                ...
                CCommerciumAddress pubKeyAddress = CCommerciumAddress(pubKey.GetID());
                ...
                // Consistency check.
                if (isScript) {
                    CCommerciumAddress scriptAddress;
                    CTxDestination destination;

                    if (ExtractDestination(script, destination)) {
                        scriptAddress = CCommerciumAddress(destination);
