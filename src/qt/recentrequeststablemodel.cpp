    ...
    if(role == Qt::DisplayRole || role == Qt::EditRole)
    {
        ...
        case Amount:
            if (rec->recipient.amount == 0 && role == Qt::DisplayRole)
                ...
                return CommerciumUnits::format(walletModel->getOptionsModel()->getDisplayUnit(), rec->recipient.amount, false, CommerciumUnits::separatorNever);
            else
                return CommerciumUnits::format(walletModel->getOptionsModel()->getDisplayUnit(), rec->recipient.amount);
        }
    }
...
{
    return (this->walletModel->getOptionsModel() != NULL) ? tr("Requested") + " ("+CommerciumUnits::name(this->walletModel->getOptionsModel()->getDisplayUnit()) + ")" : "";
}
