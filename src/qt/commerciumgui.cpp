...
CommerciumGUI::CommerciumGUI(const PlatformStyle *_platformStyle, const NetworkStyle *networkStyle, QWidget *parent) :
    QMainWindow(parent),
    enableWallet(false),
    clientModel(0),
    walletFrame(0),
...
void CommerciumGUI::createActions()
{
    ...
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and commercium: URIs)"));
    ...
    aboutAction->setStatusTip(tr("Show information about Commercium"));
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your Commercium addresses to prove you own them"));
    verifyMessageAction = new QAction(QIcon(":/icons/" + theme + "/transaction_0"), tr("&Verify message..."), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified Commercium addresses"));
    ...
    openAction->setStatusTip(tr("Open a commercium: URI or payment request"));

    showHelpMessageAction = new QAction(QApplication::style()->standardIcon(QStyle::SP_MessageBoxInformation), tr("&Command-line options"), this);
    showHelpMessageAction->setMenuRole(QAction::NoRole);
    showHelpMessageAction->setStatusTip(tr("Show the %1 help message to get a list with possible Commercium command-line options").arg(tr(PACKAGE_NAME)));
...
void CommerciumGUI::createIconMenu(QMenu *pmenu)
{
...
void CommerciumGUI::showInfo()
{
    rpcConsole->setTabFocus(RPCConsole::TAB_INFO);
    showDebugWindow();
}

void CommerciumGUI::showConsole()
{
    rpcConsole->setTabFocus(RPCConsole::TAB_CONSOLE);
    showDebugWindow();
}

void CommerciumGUI::showGraph()
{
    rpcConsole->setTabFocus(RPCConsole::TAB_GRAPH);
    showDebugWindow();
}

void CommerciumGUI::showPeers()
{
    rpcConsole->setTabFocus(RPCConsole::TAB_PEERS);
    showDebugWindow();
}

void CommerciumGUI::showRepair()
{
    rpcConsole->setTabFocus(RPCConsole::TAB_REPAIR);
    showDebugWindow();
}

void CommerciumGUI::showConfEditor()
{
    GUIUtil::openConfigfile();
}

void CommerciumGUI::showMNConfEditor()
{
    GUIUtil::openMNConfigfile();
}

void CommerciumGUI::showBackups()
{
    GUIUtil::showBackups();
}
...
void CommerciumGUI::showPrivateSendHelpClicked()
{
    if(!clientModel)
        return;
...
void CommerciumGUI::gotoMasternodePage()
{
    QSettings settings;
    if (!fLiteMode && settings.value("fShowMasternodesTab").toBool() && masternodeAction) {
...
void CommerciumGUI::updateNetworkState()
{
    ...
    if (clientModel->getNetworkActive()) {
        labelConnectionsIcon->setToolTip(tr("%n active connection(s) to Commercium network", "", count));
    } else {
...
void CommerciumGUI::setNumBlocks(int count, const QDateTime& blockDate, double nVerificationProgress, bool header)
{
    if (modalOverlay)
    {
        if (header)
    ...
    if (walletFrame)
    {
        if(secs < 25*60) // 90*60 in commercium
        {
...
void CommerciumGUI::setAdditionalDataSyncProgress(double nSyncProgress)
{
    if(!clientModel)
        return;
...
void CommerciumGUI::message(const QString &title, const QString &message, unsigned int style, bool *ret)
{
    QString strTitle = tr("Commercium"); // default title
...
#ifdef ENABLE_WALLET
void CommerciumGUI::incomingTransaction(const QString& date, int unit, const CAmount& amount, const QString& type, const QString& address, const QString& label)
{
...
static bool ThreadSafeMessageBox(CommerciumGUI *gui, const std::string& message, const std::string& caption, unsigned int style)
{
...
void CommerciumGUI::handleRestart(QStringList args)
{
    if (!ShutdownRequested())
    ...
    menu(0)
{
    ...
    Q_FOREACH (const CommerciumUnits::Unit unit, units)
    {
...
{
    menu = new QMenu(this);
    Q_FOREACH(CommerciumUnits::Unit u, CommerciumUnits::availableUnits())
    {
