...
void SendCoinsDialog::send(QList<SendCoinsRecipient> recipients, QString strFee, QString strFunds)
{
    ...
    processSendCoinsReturn(prepareStatus,
        CommerciumUnits::formatWithUnit(model->getOptionsModel()->getDisplayUnit(), currentTransaction.getTransactionFee()));
    ...
    Q_FOREACH(const SendCoinsRecipient &rcp, currentTransaction.getRecipients())
    {
        ...
        QString amount = "<b>" + CommerciumUnits::formatHtmlWithUnit(model->getOptionsModel()->getDisplayUnit(), rcp.amount);
    ...
    if(txFee > 0)
    {
        ...
        questionString.append(CommerciumUnits::formatHtmlWithUnit(model->getOptionsModel()->getDisplayUnit(), txFee));
    ...
    Q_FOREACH(CommerciumUnits::Unit u, CommerciumUnits::availableUnits())
    {
        if(u != model->getOptionsModel()->getDisplayUnit())
            alternativeUnits.append(CommerciumUnits::formatHtmlWithUnit(u, totalAmount));
    }

    // Show total amount + all alternative units
    questionString.append(tr("Total Amount = <b>%1</b><br />= %2")
        .arg(CommerciumUnits::formatHtmlWithUnit(model->getOptionsModel()->getDisplayUnit(), totalAmount))
                                 ...
                                 const CAmount& watchBalance, const CAmount& watchUnconfirmedBalance, const CAmount& watchImmatureBalance)
{
    ...
    if(model && model->getOptionsModel())
    {
        ...
        ui->labelBalance->setText(CommerciumUnits::formatWithUnit(model->getOptionsModel()->getDisplayUnit(), bal));
    }
}
...
void SendCoinsDialog::processSendCoinsReturn(const WalletModel::SendCoinsReturn &sendCoinsReturn, const QString &msgArg)
{
    ...
    case WalletModel::AbsurdFee:
        msgParams.first = tr("A fee higher than %1 is considered an absurdly high fee.").arg(CommerciumUnits::formatWithUnit(model->getOptionsModel()->getDisplayUnit(), maxTxFee));
        break;
...
void SendCoinsDialog::updateFeeMinimizedLabel()
{
    ...
    if (ui->radioSmartFee->isChecked())
        ...
        ui->labelFeeMinimized->setText(CommerciumUnits::formatWithUnit(model->getOptionsModel()->getDisplayUnit(), ui->customFee->value()) +
...
void SendCoinsDialog::updateMinFeeLabel()
{
    if (model && model->getOptionsModel())
        ui->checkBoxMinimumFee->setText(tr("Pay only the required fee of %1").arg(
            CommerciumUnits::formatWithUnit(model->getOptionsModel()->getDisplayUnit(), CWallet::GetRequiredFee(1000)) + "/kB")
        );
}
...
void SendCoinsDialog::updateSmartFeeLabel()
{
    ...
    {
        ui->labelSmartFee->setText(CommerciumUnits::formatWithUnit(model->getOptionsModel()->getDisplayUnit(),
...
void SendCoinsDialog::coinControlChangeEdited(const QString& text)
{
    if (model && model->getAddressTableModel())
    {
        ...
        CCommerciumAddress addr = CCommerciumAddress(text.toStdString());
        ...
        {
            ui->labelCoinControlChangeLabel->setText(tr("Warning: Invalid Commercium address"));
        }
        else // Valid address
        {
            CKeyID keyid;
