...
{
    GUIUtil::setClipboard(CommerciumUnits::removeSpaces(contextMenuItem->text(COLUMN_AMOUNT)));
}
...
void CoinControlDialog::updateLabels(WalletModel *model, QDialog* dialog)
{
    ...
    int nDisplayUnit = CommerciumUnits::COMMERCIUM;
    ...
    l2->setText(CommerciumUnits::formatWithUnit(nDisplayUnit, nAmount));        // Amount
    l3->setText(CommerciumUnits::formatWithUnit(nDisplayUnit, nPayFee));        // Fee
    l4->setText(CommerciumUnits::formatWithUnit(nDisplayUnit, nAfterFee));      // After Fee
    l5->setText(((nBytes > 0) ? ASYMP_UTF8 : "") + QString::number(nBytes));        // Bytes
    l7->setText(fDust ? tr("yes") : tr("no"));                               // Dust
    l8->setText(CommerciumUnits::formatWithUnit(nDisplayUnit, nChange));        // Change
    if (nPayFee > 0 && (coinControl->nMinimumTotalFee < nPayFee))
    {
    ...
    QString toolTip4 = tr("Can vary +/- %1 commercii(s) per input.").arg(dFeeVary);
...
void CoinControlDialog::updateView()
{
    ...
    BOOST_FOREACH(const PAIRTYPE(QString, std::vector<COutput>)& coins, mapCoins) {
        ...
        BOOST_FOREACH(const COutput& out, coins.second) {
            ...
            {
                sAddress = QString::fromStdString(CCommerciumAddress(outputAddress).ToString());

                // if listMode or change => show commercium address. In tree mode, address is not shown again for direct wallet address outputs
            ...
            // amount
            itemOutput->setText(COLUMN_AMOUNT, CommerciumUnits::format(nDisplayUnit, out.tx->tx->vout[out.i].nValue));
            itemOutput->setToolTip(COLUMN_AMOUNT, CommerciumUnits::format(nDisplayUnit, out.tx->tx->vout[out.i].nValue));
        ...
        if (treeMode)
        {
            ...
            itemWalletAddress->setText(COLUMN_AMOUNT, CommerciumUnits::format(nDisplayUnit, nSum));
            itemWalletAddress->setToolTip(COLUMN_AMOUNT, CommerciumUnits::format(nDisplayUnit, nSum));
